...
class Template:
    def __init__(self, path, name):
        ...
        if self.image is not None:
            ...
            # Record time for resizing
            resizing_end_time = time.perf_counter()
            resizing_time = resizing_end_time - registration_start_time
            print(f"Template resizing time: {resizing_time:.4f} seconds")
            ...
            # Start timing for SIFT feature extraction
            sift_start_time = time.perf_counter()
            self.kp, self.des = sift.detectAndCompute(self.gray, None)
            
            # Record time for SIFT extraction
            sift_end_time = time.perf_counter()
            sift_time = sift_end_time - sift_start_time
            print(f"SIFT feature extraction time: {sift_time:.4f} seconds")
            
            # Store original dimensions for scaling back
            self.original_h, self.original_w = self.image.shape[:2]

            # Record the total registration time
            registration_end_time = time.perf_counter()
            total_registration_time = registration_end_time - registration_start_time
            print(f"Total template registration time: {total_registration_time:.4f} seconds")
...
def process_template_chunk(args):
    ...
    # Start time for the matching and authentication phase
        ...
        if len(good_matches) > TEMPLATE_PARAMS['min_matches']:
            ...
            if M is not None and match_percentage >= CONFIDENCE_PARAMS['min_match_percentage']:
                ...
                if confidence >= CONFIDENCE_PARAMS['min_confidence']:
                    # Authentication time after the confidence threshold is met
                    auth_end_time = time.perf_counter()
                    auth_time = auth_end_time - auth_start_time
                    print(f"[Profiling] Authentication Time: {auth_time:.4f} seconds")

                    # Convert corners to rectangle format
                    h, w = template.original_h, template.original_w
                    pts = np.float32([[0, 0], [0, h-1], [w-1, h-1], [w-1, 0]]).reshape(-1, 1, 2)

                    dst = cv2.perspectiveTransform(pts, M)
                    results.append((template, match_percentage, dst, confidence))
